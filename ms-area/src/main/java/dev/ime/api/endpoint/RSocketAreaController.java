...
@Controller
public class RSocketAreaController implements RSocketControllerPort{

	private final ReactiveLoggerUtils reactiveLoggerUtils;
	private final RSocketServicePort rSocketServicePort;

	public RSocketAreaController(ReactiveLoggerUtils reactiveLoggerUtils, RSocketServicePort rSocketServicePort) {
		super();
		this.reactiveLoggerUtils = reactiveLoggerUtils;
		this.rSocketServicePort = rSocketServicePort;
	}
	
	@MessageMapping("areas.existsById.{id}")
	@Override
	public Mono<Boolean> existsById(@DestinationVariable("id") String areaId) {
		...
		return Mono.justOrEmpty(areaId)
				...
				.flatMap(this::existsByIdInDatabase)
	...
	private Mono<Boolean> existsByIdInDatabase(UUID id){
